    ...
    return (
      <div className="space-y-6">
        <div className="flex items-start justify-between">
          <div>
            <div className="skeleton-pulse h-4 w-24 rounded bg-navy-200" />
            <div className="skeleton-pulse mt-2 h-8 w-48 rounded bg-navy-200" />
          </div>
          <div className="skeleton-pulse h-20 w-40 rounded-lg bg-navy-200" />
        </div>
    ...
    <div className="space-y-8">
      ...
      {isEmployee && (
        <StaggerGrid className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { href: "/accounts/create", icon: Plus, label: "New Account", desc: "Open an account", color: "bg-navy-100 text-navy-700 group-hover:bg-navy-700 group-hover:text-white" },
            { href: "/transactions/deposit", icon: TrendingUp, label: "Deposit", desc: "Cash deposit", color: "bg-brand-100 text-brand-700 group-hover:bg-brand-500 group-hover:text-white" },
            { href: "/customers/create", icon: Plus, label: "New Customer", desc: "Register customer", color: "bg-navy-100 text-navy-700 group-hover:bg-navy-700 group-hover:text-white" },
            { href: "/loans", icon: HandCoins, label: "Loans", desc: "Manage loans", color: "bg-brand-100 text-brand-700 group-hover:bg-brand-500 group-hover:text-white" },
          ].map((item) => (
            <StaggerItem key={item.href}>
              <motion.div whileHover={{ y: -3 }} whileTap={{ scale: 0.98 }}>
                <Link href={item.href} className="group card-easytrust flex items-center gap-4 p-5">
                  <div className={`flex h-11 w-11 items-center justify-center rounded-lg transition-all ${item.color}`}>
        ...
        <StaggerGrid className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <StaggerItem key={item.href}>
              <motion.div whileHover={{ y: -3 }} whileTap={{ scale: 0.98 }}>
                <Link href={item.href} className="group card-easytrust flex items-center gap-4 p-5">
            <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-navy-100 text-navy-600 transition-all group-hover:bg-navy-900 group-hover:text-white shadow-sm">
              <item.icon className="h-5 w-5" />
            </div>
            <div>
              <p className="text-sm font-semibold text-navy-900">{item.label}</p>
      ...
      <div>
        <div className="flex items-center justify-between mb-5">
          ...
          <span className="text-xs text-navy-400 font-mono tracking-wider">{accounts?.length ?? 0} account{(accounts?.length ?? 0) !== 1 ? "s" : ""}</span>
        </div>
            ...
            {accounts.slice(0, 6).map((account) => (
              <StaggerItem key={account.account_id}>
                <motion.div whileHover={{ y: -4 }} whileTap={{ scale: 0.98 }}>
                  <Link href={`/accounts/${account.account_id}`} className="group card-easytrust overflow-hidden block">
                    <div className="p-6 pb-20">
                      <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-background text-navy-600 group-hover:bg-navy-900 group-hover:text-white transition-all">
                        ...
                        <div>
                          ...
                          <p className="font-mono text-xs text-navy-400 tracking-wider">
                    ...
                    <div className="gradient-card px-6 py-4">
                      ...
                      <p className="denomination text-xl text-white">
                        {formatCurrency(Number(account.balance), account.currency)}
                      </p>
                      <div className="denomination-rule mt-2" />
